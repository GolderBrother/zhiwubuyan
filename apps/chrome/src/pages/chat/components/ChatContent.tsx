...
import { App as AntdApp, Flex, Button, Space, Badge, Collapse, Tooltip } from 'antd';
import { Bubble } from '@ant-design/x';
import { FileCopyIcon, RefreshIcon, Filter3Icon, SoundIcon, PauseCircleIcon } from 'tdesign-icons-react';
      ...
      <Bubble.List
        ...
        items={messages.map(({ role, content, model }, i) => {
          return {
            ...
            footer:
              role === 'assistant' && responseStatus === 'end' ? (
                <Space>
                  <Tooltip title={isSpeaking ? '暂停朗读' : '语音朗读'}>
                    <Button
                      color="default"
                      variant="text"
                      size="small"
                      icon={isSpeaking ? <PauseCircleIcon /> : <SoundIcon />}
                      onClick={() => {
                        handleSpeak(content);
                      }}
                    />
